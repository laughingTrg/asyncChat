...
import select
import argparse
from sqlalchemy import Integer, String, Column, ForeignKey, create_engine, Table, MetaData, text
...
class ClientHistoryDB(Base):
    ...
    def __repr__(self):
        return "<CLient('%s', '%s')>" % (self.login_time, self.ip_addr)
...
class ClientContactListDB(Base):
    ...
    def __repr__(self):
        return "<CLient('%s', '%s')>" % (self.id_owner, self.id_client)
